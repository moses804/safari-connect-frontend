...
const AccommodationBookingForm = ({ accommodation, onSuccess }) => {
  ...
  const handleDateChange = (e) => {
    ...
    setFormData(newData);
    setEstimatedPrice(calculatePrice(newData.check_in_date, newData.check_out_date));
  };
  ...
  if (!user) {
    return (
      <div className="p-6 bg-blue-50 border-l-4 border-blue-500 rounded-lg">
        <p className="text-blue-900 font-semibold">Please log in to book this accommodation</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 bg-white rounded-lg border border-gray-200 shadow">
      <h3 className="text-2xl font-bold text-gray-900 mb-6">Book This Accommodation</h3>
